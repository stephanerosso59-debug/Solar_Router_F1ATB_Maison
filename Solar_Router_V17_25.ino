...
//Ethernet
#include "GpioPinConfig.h"   // pinout PCB Maison (W5500 SPI + Triac/JSY/LED/Temp/Fan)
#include "FanControl.h"      // PCB Maison : ventilateur PWM (GPIO33)
int16_t EthernetBug = 0;
// --- PCB Maison : W5500 en SPI au lieu du LAN8720 natif F1ATB ---
W5500Driver driver(GPIO_W5500_CS, GPIO_W5500_INT);   // CS=5, INT=25
//EMACDriver driver(ETH_PHY_LAN8720, 23, 18, 16);    // (original F1ATB WT32-ETH01)
...
void setup() {
  ...
  init_puissance();
  InitTemperature();
  initFan();                 // PCB Maison : ventilateur PWM (GPIO33)
  ...
  MessageCommandes();
  LireSerial();
  // --- PCB Maison : bus SPI custom pour le W5500 (CLK=14, MISO=15, MOSI=13, CS=5) ---
  SPI.begin(GPIO_W5500_CLK, GPIO_W5500_MISO, GPIO_W5500_MOSI, GPIO_W5500_CS);
...
void loop() {
  ...
  if (tps - previousTempMillis > 15001) {
    ...
    //Temperature
    LectureTemperature();
    // --- PCB Maison : ventilateur (piloté par la temp la plus haute) + alerte sécurité ---
    {
      float tMax = -127;
      for (int c = 0; c < 4; c++)
        if (TemperatureValide[c] > 0 && temperature[c] > tMax) tMax = temperature[c];
      if (tMax > -100) {
        updateFanSpeed(tMax);
        if (tMax >= 75.0) {
          StockMessage("[SECURITE] Temperature " + String(tMax) + " C >= 75 C !");
          // TODO (à adapter selon ton câblage) : action d'urgence (coupure Triac/SSR)
        }
      }
    }
...
void InitGPIOs() {
  ...
  if (pTriac > 0) {
    ...
    if (ESP32_Type == 10) pTriac = 2;                    //PCB Maison W5500 : gâchette GPIO22 + ZC GPIO23 (VERROUILLÉ, ignore le menu déroulant)
