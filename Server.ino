...
void Init_Server() {
  ...
  // --- OTA retire (PCB Maison) : partition custom sans slot OTA -> MAJ uniquement par USB/Arduino ---
  // (handlers /OTA et /update supprimes)
...
void handlePinsActionsJS() {  // Pins disponibles
  ...
  if (ESP32_Type == 10)
    S = "var Pins=[0,21,-1];";  // PCB Maison : SEUL le SSR (GPIO21) sélectionnable pour une Action/Relais (VERROUILLÉ). Triac=22/ZC=23 forcés en code (pTriac=2).
  //if (ESP32_Type == 10) S = "var Pins=[0,5,12,14,17,32,33,-1];";  // (original WT32-ETH01 LAN8720)
...
void handleCommunCSS() {
  ...
  // PCB Maison : theme sombre FORCE sur toutes les pages (ignore la couleur configurable pour le fond)
  S += "body {font-size:150%;text-align:center;width:100%;max-width:1000px;margin:auto;padding:10px;background:#0f172a;background-attachment:fixed;color:#e2e8f0;}\n";
