...
function SetParaFixe() {
    ...
    // --- PCB Maison : GPIO fixes → menus affichés mais VERROUILLÉS (grisés, non modifiables) ---
    // Le F1ATB sauve via GID().value (lisible même désactivé) → la config reste correcte.
    GID("pTriac").value = 2;  GID("pTriac").disabled = true;   // Triac : Pulse=22, Zc=23 (câblé)
    GID("pLED").value = 1;    GID("pLED").disabled = true;     // LEDs vertes/rouges 18/19 (câblé)
